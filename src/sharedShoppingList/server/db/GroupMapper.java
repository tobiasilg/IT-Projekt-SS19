...
public class GroupMapper {
	...
	/**
	 * Falls noch kein GroupMapper existiert wird ein neuen GroupMapper erstellt und
	 * zurückgegeben
	 *
	...
	/**
	 * Findet Groups durch eine G_ID und speichert die dazugehörigen Werte (G_ID,
	 * name, createDate, modDate) in einem Group Objekt ab und gibt dieses wieder
	 *
	...
	public Group findByID(Group group) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT G_ID, name, createDate, modDate FROM T_Group WHERE G_ID ="
					+ group.getId() + " ORDER BY modDate");
	...
	/**
	 * Gibt alle Group Objekte eine spez. Users zurück welche mit G_ID, name,
	 * createDate und modDate befüllt sind
	 *
	...
	public Vector<Group> findAllByUID(Group g) {
		...
		try {
			Statement stmt = con.createStatement();
			ResultSet rs = stmt.executeQuery("SELECT G_ID, name, createDate, modDate FROM T_Group WHERE creator ="
					+ g.getId() + " ORDER BY modDate");
	...
	public Group insert(Group group) {
		...
		try {
			...
			if (rs.next()) {
				...
				stmt2.executeUpdate("INSERT INTO T_Group (G_ID, name, createDate, modDate)" + " VALUES ("
						+ group.getId() + ", " + group.getName() + ", '" + g + "', '" + g + "')");
	...
	/**
	 * Befüllt T_Group mit G_ID, name, createDate und modDate, falls sich was
	 * geändert hat Ein Group Objekt wird zurückgegeben
	 *
	...
	public Group update(Group group) {
		...
		try {
			Statement stmt = con.createStatement();
			stmt.executeUpdate("UPDATE T_Group SET G_ID =" + group.getId() + ", creator =" + group.getName()
// +"', createDate="
// + group.getCreateDate()
					+ "', modDate='" + g + "' WHERE G_ID=" + group.getId());
		} catch (SQLException e2) {
			e2.printStackTrace();
			return group;
		}
	...
	/**
	 * Entfernt alles aus T_Group wo die G_ID der ID des übergebenen Objekts
	 * entspricht
	 *
